---
name: triz
description: Use when an inventive or hard problem is blocked by a trade-off / contradiction and the obvious answer is a compromise — in product, engineering, software/UX, or design. Runs the TRIZ method (IFR-first 5-question spine; resolves contradictions via separation/function tools without compromise, instead of splitting the difference). Triggers include "X fights Y", "we keep trading off A vs B", "resolve this contradiction", "apply TRIZ", "inventive / non-compromise solution to".
---

<!-- This skill mirrors the canonical method at docs/triz-method-skill.md (kept in sync).
     Reference data: data/parameters.json (39 parameters + alias thesaurus, engineering-only),
     data/principles.json (40 inventive principles). Deep knowledge: query vault/ with vaultmind. -->
